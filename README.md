...
This project implements the **Week 1 multimodal ingestion and vector database pipeline** for the OmniBrain AI system. The pipeline processes PDF documents, extracts text and images, generates embeddings, and stores them in **Qdrant** for retrieval-based AI applications.
...
* Detect and rasterize table regions as images
* Split text into overlapping chunks
* Generate a structured `manifest.json`
* Create text embeddings
* Create image embeddings
* Store embeddings in Qdrant
* Verify retrieval from the vector database
...
* Extracted text from PDF files using **PyMuPDF**
* Extracted embedded figures and chart images from PDF pages

## 2. Table Extraction

* Detected table regions using **pdfplumber**
* Rasterized tables as PNG images using **PyMuPDF**
* Stored tables as image modality for Vision-Language Model processing

## 3. Text Chunking

* Split document text into overlapping chunks using **RecursiveCharacterTextSplitter**
* Excluded table regions from prose text to avoid duplicate information

## 4. Manifest Generation

Generated a structured `manifest.json` containing:

* text chunks
* extracted images
* table images
* metadata such as page number, source file, and file paths

---

# Vector Database Setup (Qdrant)

Implemented a multimodal vector database pipeline using **Qdrant**.
...
Final verification after insertion into Qdrant:
...
Successful semantic image retrieval test:

```text
Query: "revenue chart"
Relevant table/image retrieved successfully.
```
...
```bash
pip install qdrant-client sentence-transformers pillow torch clip
```
...
## Step 5: Store image embeddings

```bash
python embed_images.py
```

## Step 6: Verify stored vectors
...
Successfully built a **multimodal ingestion and vector storage pipeline** that supports:

* text retrieval
* image retrieval
* table-as-image retrieval
* metadata-based citation and traceability

This pipeline forms the foundation for the next stage of the OmniBrain project: **LangGraph-based multimodal retrieval and reasoning**.
...
| Table Rasterization     | Completed |
| Text Chunking           | Completed |
| Manifest Generation     | Completed |
| Qdrant Setup            | Completed |
| Text Embeddings Stored  | Completed |
| Image Embeddings Stored | Completed |
| Retrieval Verification  | Completed |


# Author

**Pramod**
OmniBrain Project — Week 1 Vector Database Setup

...
def run_query_pipeline(prompt: str, vision_image, rails, process_image, build_graph) -> tuple[dict, dict]:
    """
    ...
    if vision_image is not None:
        ...
        graph = build_graph()
        result = graph.invoke({"messages": [prompt], "task": prompt,"step_count": 0,})
...
for message in st.session_state.messages:
    ...
    with st.chat_message(message["role"]):
        st.markdown(message["content"])
        # Display citations if available in message
            if "sources" in message and message["sources"]:
                st.markdown("---")
                st.caption("📌 **Source Citations:**")
                for src in message["sources"]:
                    file_name = src.get("source_file", "Document.pdf")
...
if user_prompt:
    ...
    with st.chat_message("assistant"):
    with st.status(
        "OmniBrain pipeline and safety guardrails active...",
        expanded=True
    ) as status:
        ...
        try:
            ...
            status.update(
                label="Execution error encountered",
                state="error",
                expanded=True
            )
    ...
    if isinstance(assistant_response, dict):
        ...
        result_text = assistant_response.get("result", "")

        if result_text:
            st.markdown(result_text)

        # -----------------------------
        # Task 4: Visual Source Mapping
        # -----------------------------
        visual_sources = assistant_response.get(
            "visual_sources",
            []
        )
        ...
        if visual_sources:
            ...
            for source in visual_sources:
                ...
                if kind == "table_region":
                    st.markdown("### 📋 Referenced Table")
                else:
                    st.markdown("### 📊 Referenced Chart / Figure")
                ...
                if image_path:
                    ...
                    if full_image_path.exists():
                        ...
                        st.warning(
                            f"Visual file not found: {image_path}"
                        )
